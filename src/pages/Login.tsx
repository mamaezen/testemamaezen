      ...
      <div className="flex-1 flex flex-col items-center justify-center px-5 py-8 relative z-10">
        ...
        <div className="w-full max-w-sm space-y-4">
          <div className="relative group">
            {/* Neon glow border */}
            <div className="absolute -inset-[1px] rounded-3xl bg-gradient-to-b from-primary/60 via-primary/20 to-primary/60 opacity-80 blur-[1px] group-hover:opacity-100 transition-opacity" />
            <div className="relative p-6 rounded-3xl bg-card/80 backdrop-blur-2xl border border-primary/10 shadow-[0_0_40px_-10px_hsl(var(--primary)/0.3)] space-y-5">
              <div className="text-center space-y-1">
                <p className="text-base font-bold text-foreground tracking-tight">
                  {isUSA ? 'Start now' : 'Comece agora'}
                </p>
                <p className="text-xs text-muted-foreground">
                  {isUSA ? 'Sign in to access all features' : 'Entre para acessar todos os recursos'}
                </p>
              </div>
              ...
              <Button
                ...
                className="w-full h-14 bg-gradient-to-r from-primary to-primary/80 hover:from-primary hover:to-primary text-primary-foreground font-bold text-base rounded-2xl shadow-[0_0_30px_-5px_hsl(var(--primary)/0.5)] hover:shadow-[0_0_40px_-5px_hsl(var(--primary)/0.7)] transition-all duration-300 hover:scale-[1.02] active:scale-[0.98] disabled:opacity-70 gap-3"
                size="lg"
              >
                {isLoggingIn ? (
                ...
                {!isLoggingIn && <ChevronRight className="w-5 h-5 ml-auto" />}
              </Button>

              {/* Divider */}
              <div className="flex items-center gap-4">
                <div className="flex-1 h-px bg-gradient-to-r from-transparent via-primary/30 to-transparent" />
                <span className="text-[11px] text-muted-foreground/60 uppercase tracking-widest font-medium">{isUSA ? 'or' : 'ou'}</span>
                <div className="flex-1 h-px bg-gradient-to-r from-transparent via-primary/30 to-transparent" />
              </div>
              ...
              {!showKeyInput ? (
                <Button
                  ...
                  className="w-full h-14 rounded-2xl border-primary/20 bg-primary/5 text-foreground hover:bg-primary/10 hover:border-primary/40 transition-all duration-300 gap-3 text-base font-semibold"
                >
                ...
                <div className="space-y-3">
                  <Input
                    placeholder={isUSA ? 'XXXXX-XXXXX-XXXXX-XXXXX' : 'XXXXX-XXXXX-XXXXX-XXXXX'}
                    value={licenseKey}
                    onChange={e => setLicenseKey(e.target.value.toUpperCase())}
                    maxLength={50}
                    className="h-14 rounded-2xl bg-muted/30 border-primary/20 focus:border-primary/50 text-center font-mono tracking-widest text-sm"
                  />
                  <Button
                    onClick={async () => {
                    ...
                    className="w-full h-14 bg-gradient-to-r from-primary to-primary/80 hover:from-primary hover:to-primary text-primary-foreground font-bold rounded-2xl shadow-[0_0_30px_-5px_hsl(var(--primary)/0.5)] gap-3 text-base"
                  >
                    {activatingKey ? (
                      <Sparkles className="w-5 h-5 animate-spin" />
                    ) : (
          ...
          <div className="flex items-center justify-between px-1">
            ...
            <div className="rounded-2xl bg-card/30 backdrop-blur-sm border border-primary/10 px-2.5 py-1">
              <CountrySelector />
            </div>
          </div>
        </div>
